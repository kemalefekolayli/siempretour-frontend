...
TARGET_ROOT = os.path.join(BASE_DIR, "siempre_tour_safari_tours")

TARGET_CATEGORY = "Safari"
...
def export_safari_tours():
    ...
    for country in os.listdir(SOURCE_ROOT):
        ...
        safari_tours = [t for t in tours if t.get("category") == TARGET_CATEGORY]

        # Bu ülkede Safari turu yoksa skip
        if not safari_tours:
            continue
        ...
        save_json(out_tours_path, safari_tours)

        countries_created += 1
        total_tours_exported += len(safari_tours)

        print(f"🦁 {country}: {len(safari_tours)} Safari turu -> {out_tours_path}")

    print("\n==============================")
    print(f"✅ Ülke klasörü oluşturulan: {countries_created}")
    print(f"✅ Export edilen toplam Safari turu: {total_tours_exported}")
